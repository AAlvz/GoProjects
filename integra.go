...
///***Funcion
///...AdivinaPconX ... devuelve el valor x para un valor buscado p
func AdivinaPconX(num_seg, pBusc, dof float64)(float64){
	x := 1.0
	d := .5
	num_seg = 10.0
	p := Simpson(num_seg, x, dof)
	for round.Round(p, 5) != pBusc{
                for p < pBusc{
			x = x + d
			p = Simpson(num_seg, x, dof)
                }///...finFor
	...
	return round.Round(x, 5)
}///...FinFunc

///***Funcion
///... obtiene el valor de la cola de la graf
func GetCola(x, dof float64) float64{
//	p := Simpson(10.0, x, dof)
//	tail := 1 - 2*p
//	return tail
	return 1 - (2 * Simpson(10.0, x, dof))
}///...FunFunc
